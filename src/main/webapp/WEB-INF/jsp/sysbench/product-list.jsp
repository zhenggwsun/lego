    ...
    <div class="page-content">
        <div class="content">
            ...
            <xblock><!--<button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button>-->
                <a href="/sysbench/product/export"><button class="layui-btn" onclick=""><i class="layui-icon">&#xe64c;</i>数据导出</button></a>
                <button class="layui-btn" onclick="member_add('添加产品','/sysbench/productAdd','600','500')"><i class="layui-icon">&#xe608;</i>添加</button>
                <span class="x-right" style="line-height:40px">共有数据：${requestScope.PaginationModel.total} 条</span></xblock>
            <table class="layui-table">
                ...
                <tr>
                    ...
                    <td>
                        <img src="${pageContext.request.contextPath}/uploaded/${product.photo}"  width="50" height="50" class="img-responsive" />
                    </td>
                    <td >
                            ${product.productName}
                    </td>
                    ...
                    <td>
                             <fmt:formatDate value="${product.gmtCreate}" pattern="yyyy/MM/dd  HH:mm:ss" />
                    </td>
                    <td class="td-manage">
                        <a title="编辑" href="javascript:;" onclick="member_edit('编辑','/sysbench/product/queryProductInfo?productId=${product.id}','4','','510')"
                           class="ml-5" style="text-decoration:none">
                            <i class="layui-icon">&#xe642;</i>
                        </a>
                        <a title="删除" href="javascript:;" onclick="member_del(this,${product.id})"
        ...
        <xblock>
            <c:choose>
                <c:when test="${requestScope.PaginationModel.pageNum > 1}">
                    <a href="/sysbench/product/queryProductList?pageNum=${param.pageNum-1}&pageSiz=10"><button class="layui-btn">上一页</button></a>
                </c:when>
                <c:otherwise>
                    <a href="/sysbench/product/queryProductList?pageNum=${param.pageNum-1}&pageSiz=10"><button class="layui-btn" style="color: grey" disabled="disabled">上一页</button></a>
                </c:otherwise>
            </c:choose>
            <c:choose>
                <c:when test="${requestScope.PaginationModel.pageNum < requestScope.PaginationModel.totalPages}">
                    <a href="/sysbench/product/queryProductList?pageNum=${param.pageNum+1}&pageSiz=10"><button class="layui-btn">下一页</button></a>
                </c:when>
                <c:otherwise>
                    <a href="/sysbench/product/queryProductList?pageNum=${param.pageNum+1}&pageSiz=10"><button class="layui-btn" style="color: grey" disabled="disabled">下一页</button></a>
                </c:otherwise>
            </c:choose>
            <c:choose>
        ...
        layer.confirm('确认要删除吗？',function(index){
            ...
            $.post("/sysbench/product/delete",{
                ...
                if (data.success == true){
                    ...
                    layer.msg('系统异常!',{icon:1,time:1000});
                }
            });
