...
<div class="wrapper">
    ...
    <div class="page-content">
        <div class="content">
            <!-- 右侧内容框架，更改从这里开始 -->
            <form class="layui-form xbs" action="/sysbench/repertory/queryList" method="get">
                <div class="layui-form-pane" style="text-align: center;">
                    <div class="layui-form-item" style="display: inline-block;">
                        <label class="layui-form-label xbs768">商品id</label>
                        <div class="layui-input-inline">
                            <input type="text" id="product-id" name="productId"  placeholder="请输入商品id" autocomplete="off" class="layui-input">
                        </div>
            ...
            <xblock>
                <button class="layui-btn" onclick="member_add('添加库存','/sysbench/repertoryAdd','600','500')"><i class="layui-icon">&#xe608;</i>添加</button>
            ...
            <table class="layui-table">
                ...
                <tr>
                    ...
                    <td >
                            ${model.forSaleCount}双
                    </td>
                    <td >
                            ${model.productId}
                    </td>
                    <td class="td-manage">
                        <a title="编辑" href="javascript:;" onclick="member_edit('编辑','/sysbench/repertory/queryInfo?id=${model.id}','4','','510')"
                           class="ml-5" style="text-decoration:none">
                            <i class="layui-icon">&#xe642;</i>
                        </a>
                        <a title="删除" href="javascript:;" onclick="member_del(this,${model.id})"
        ...
        <xblock>
            <c:choose>
                <c:when test="${requestScope.PaginationModel.pageNum > 1}">
                    <a href="/sysbench/repertory/queryList?productId=${param.productId}&pageNum=${param.pageNum-1}&pageSiz=10"><button class="layui-btn">上一页</button></a>
                </c:when>
                <c:otherwise>
                    <a href="/sysbench/repertory/queryList?productId=${param.productId}&pageNum=${param.pageNum-1}&pageSiz=10"><button class="layui-btn" style="color: grey" disabled="disabled">上一页</button></a>
                </c:otherwise>
            </c:choose>
            <c:choose>
                <c:when test="${requestScope.PaginationModel.pageNum < requestScope.PaginationModel.totalPages}">
                    <a href="/sysbench/repertory/queryList?productId=${param.productId}&pageNum=${param.pageNum+1}&pageSiz=10"><button class="layui-btn">下一页</button></a>
                </c:when>
                <c:otherwise>
                    <a href="/sysbench/repertory/queryList?productId=${param.productId}&pageNum=${param.pageNum+1}&pageSiz=10"><button class="layui-btn" style="color: grey" disabled="disabled">下一页</button></a>
                </c:otherwise>
            </c:choose>
            <c:choose>
...
<script>
    /*$(function  () {
        $('#search').click(function(){
            ...
            $.get("productId="+productId+"&pageNum=1&pageSize=10",
    ...
    function member_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("/sysbench/repertory/delete",{
                id:id
            },function (data,status) {
